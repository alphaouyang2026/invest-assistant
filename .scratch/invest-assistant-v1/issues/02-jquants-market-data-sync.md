...
- [ ] Alembic 迁移建立行情数据模块的 4 张表 `instruments`、`segment_periods`、`daily_bars`、`trading_calendar`，字段按设计 §3.1–§3.4；金额与价格用 `Decimal`、在库里存文本；日期存 ISO 文本 `YYYY-MM-DD`
...
- [ ] 行情数据模块只暴露附录 A.1 的 interface；本票实现 `sync` / `read` / `calendar` / `overview`（`universe` 在 03 补上，也放在这个模块里）；`read` 一次取整批证券；其他模块不直接访问这 4 张表，也不自己算研究价格

...
**Design:** [invest-assistant v1 系统设计](../spec.md) §2–§4、§8、§9（数据页部分）、§10
...
- [ ] 行情数据模块的读取函数按设计 §4.5 一次取整批证券；其他模块不直接访问这 4 张表
- [ ] 后台任务按设计 §8：进程内单任务线程与队列、`var/job.lock` 跨进程文件锁、每个开市日 18:00 排入同步、「当天数据未出」时每 30 分钟重试到 21:00；定时逻辑用可替换的时钟测试
